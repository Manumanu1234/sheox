    ...
    return new Promise((resolve, reject) => {
      ...
      db.get().collection(admincoll.productcolletion).insertOne(body).then((result) => {
        ...
        var gotid = id.toString()
         db.get().collection(admincoll.DISCOUNT).deleteMany().then(()=>{
          db.get().collection(admincoll.OFFANDPRICE).deleteMany().then(()=>{
              callback(gotid);
          })
...
//  },
  ...
  veryfiylogin: (body) => {
    return new Promise(async (resolve, reject) => {
      if(body.email !="manumanuvkm123@gmail.com"){
        resolve({ status: false })
      }else{
           console.log('body.pass:', body.password);
      console.log('emailmached')
      if (body.email=="manumanuvkm123@gmail.com") {
          if (body.password=="manukuttan") {
            resolve({ status: true })
          } else {
            resolve({ status: false })
          }
